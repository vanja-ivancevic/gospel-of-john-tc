...
cross-witness collation: the IGNTP/INTF *Editio Critica Maior* apparatus, 214 Greek witnesses
...
A manuscript earns its weight through its pedigree. This build collates 214 Greek witnesses (the full
ECM apparatus catalogues more; this is the continuous-text set in the apparatus files used here), and a
typical verse is attested by around 140, roughly three-quarters of them near-identical Byzantine copies
that descend from a shared ancestor and echo one testimony. Counting them all as independent voices
...
a verse, measured only where at least two families are extant. The plain head-count version is kept
alongside for comparison, because the choice to treat the Byzantine majority as one voice is the
mainstream critical-text position and serious scholars contest it. Because Byzantine witnesses dominate
almost everywhere, the effective number of independent family voices is only about 1.6 gospel-wide; the
two measures barely differ on average, yet they reshuffle which verses look firm.
...
- **Chapter 21, often read as a late appendix, shows no distinctive textual instability.** It was
  copied about as faithfully as the rest of John, mid-pack on the family-vote metric and high on the
  raw count, and once confounds are controlled it is not distinctive at all. That cuts against any
  expectation that an appended ending should be textually turbulent in transmission.
...
Confounds (verse position, coverage, Synoptic parallels) explain only a few percent of the variance, so
most variation is unit-by-unit rather than structural; the surviving effects are small. Full numbers,
with confound controls and robustness checks, land in [`reports/REPORT.md`](reports/REPORT.md)
(auto-generated).
...
The pipeline is seeded and deterministic, so a clean rebuild produces byte-identical output, and it
aborts rather than publish if a validation gate fails. Findings have to survive the interpolation gates,
confound-controlled regression (HC3 robust errors, FDR over the focal hypotheses), and robustness checks
(leave-one-family-out, cluster bootstrap intervals).

The family labels used throughout are the **published** lists (f1 and f13 from the IGNTP *iohannes*
edition, a manual Alexandrian core); the project does not claim to rediscover them from scratch. The
clustering run on our own collation reproduces the tight f1 and f13 cores and separates the Byzantine
mass, and silhouette and bootstrap scores test whether the coherence distance metric is sound. It does
